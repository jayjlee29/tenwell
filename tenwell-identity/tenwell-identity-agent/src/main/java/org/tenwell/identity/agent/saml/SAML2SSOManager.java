...
public class SAML2SSOManager {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SAML2SSOManager.class);
    ...
    public String buildRedirectRequest(HttpServletRequest request, boolean isLogout) throws SSOAgentException {
        ...
        if (!isLogout) {
            ...
            AgentSessionBean sessionBean = (AgentSessionBean) request.getSession(false).
        ...
        //idpUrl = idpUrl.concat("&").concat(SSOAgentConstants.SSOAgentConfig.SAML2.HTTP_BINDING).concat("=HTTP-Redirect");
        return idpUrl;
    }
    ...
    public String buildPostRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!isLogout) {
            ...
            if (request.getSession(false) != null 
                ...
                requestMessage = buildLogoutRequest(sessionBean.getSAML2SSO().getSubjectId(), sessionBean.getSAML2SSO().getSessionIndex());
        ...
        //Add any additional parameters defined(properties)
        if (ssoAgentConfig.getQueryParams() != null && !ssoAgentConfig.getQueryParams().isEmpty()) {
            paramsMap.putAll(ssoAgentConfig.getQueryParams());
        }

        //Add any additional parameters user defined(for GET method)
        String queryParamsString="";
		if(request.getQueryString()!=null && !request.getQueryString().isEmpty()){
			try {
				queryParamsString = URLEncoder.encode(request.getQueryString(), "UTF-8");
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
				LOGGER.error("queryParamsString encode error", e);
			}
		}
        ...
        params.add(requestURL);
        paramsMap.put("requestURL", params.toArray(new String[params.size()]));
        
        params = new ArrayList<String>();
        params.add(queryParamsString);
        paramsMap.put("queryString", params.toArray(new String[params.size()]));
        ...
        if (htmlPayload == null || !htmlPayload.contains("<!--$saml_params-->")) {
        	StringBuffer htmlPayloadBuffer = new StringBuffer();
        	htmlPayloadBuffer.append("<html>\n");
        	htmlPayloadBuffer.append("<header>");
        	htmlPayloadBuffer.append("<meta http-equiv='Content-Type' content='text/html; charset=UTF-8'>");
        	htmlPayloadBuffer.append("<link rel='stylesheet' type='text/css' href='/css/default.css'>");
        	htmlPayloadBuffer.append("</header>");
        	htmlPayloadBuffer.append("<body>\n");
        	htmlPayloadBuffer.append("<p></p>");
        	htmlPayloadBuffer.append("<div class='loading'>");
        	htmlPayloadBuffer.append("<div class='loading-content'>");
        	htmlPayloadBuffer.append("<form method='post' action='" + ssoAgentConfig.getSAML2().getIdPURL() + "'>\n");
        	htmlPayloadBuffer.append("<p>You are now redirected back to " + ssoAgentConfig.getSAML2().getIdPURL() + " \n");
        	htmlPayloadBuffer.append("If the redirection fails, please click the post button.</p>\n");
        	htmlPayloadBuffer.append(htmlParams.toString());
        	htmlPayloadBuffer.append("<button type='submit' class='load-submit'>POST</button>\n");
        	htmlPayloadBuffer.append("</form>\n");
        	htmlPayloadBuffer.append("<script type='text/javascript'>\n");
        	htmlPayloadBuffer.append("document.forms[0].submit();\n");
        	htmlPayloadBuffer.append("</script>\n");
        	htmlPayloadBuffer.append("</div>");
        	htmlPayloadBuffer.append("</div>");
        	htmlPayloadBuffer.append("<span class='load-bg'></span>");
        	htmlPayloadBuffer.append("</body>\n");
        	
        	htmlPayloadBuffer.append("</html>");
            
            
            htmlPayload = htmlPayloadBuffer.toString();
        } else {
    ...
    public void processResponse(HttpServletRequest request, HttpServletResponse response)
        ...
        if (saml2SSOResponse != null) {
            ...
            LOGGER.debug("saml2SSOResponse : {}", decodedResponse);
    ...
    public void doSLO(HttpServletRequest request) throws SSOAgentException {
        ...
        if (saml2Object instanceof LogoutRequest) {
            ...
            LOGGER.debug("Received  LogoutRequest Message {}", sessionIndex);
        	...
        	LOGGER.debug("Received Logout Response Message");
            if (request.getSession(false) != null) {
                /**
                ...
                for (HttpSession session : sessions) {
                    try {
                        ...
                        LOGGER.error("LogoutResponse Ignoring exception : {}", ignore);
                    }
                }
                /*
                 * jglee 2016.12.07
                 * 서버 이중화 및 세션 로컬 스토리지 이용시 위 코드가 정상적으로 동작하지 않을수 있음
                 * */
                if(request.getSession(false)!=null){
    ...
    protected void processSSOResponse(HttpServletRequest request) throws SSOAgentException {
        ...
        AgentSessionBean sessionBean = new AgentSessionBean();
        ...
        ((AgentSessionBean) request.getSession().getAttribute(
        ...
        if (ssoAgentConfig.getSAML2().isSLOEnabled()) {
            ...
            LOGGER.debug("Add Authenticated Session in Global Context SsoSessionMap {}", sessionId);
        }

        request.getSession().setAttribute(SSOAgentConstants.SESSION_BEAN_NAME, sessionBean);
        
        LOGGER.debug("Authenticated Session Bean");
    ...
    protected AuthnRequest buildAuthnRequest(HttpServletRequest request) throws SSOAgentException {
        ...
        if(LOGGER.isDebugEnabled()){
        	LOGGER.debug(SSOAgentUtils.marshall(authRequest));
        }
    ...
    protected String encodeRequestMessage(RequestAbstractType requestMessage, String binding)
        ...
        try {
            ...
            if (SAMLConstants.SAML2_REDIRECT_BINDING_URI.equals(binding)) {
                ...
                LOGGER.debug("Unsupported SAML2 HTTP Binding. Defaulting to {}", SAMLConstants.SAML2_POST_BINDING_URI);
                return Base64.encodeBytes(rspWrt.toString().getBytes(),
                        Base64.DONT_BREAK_LINES);
            }
